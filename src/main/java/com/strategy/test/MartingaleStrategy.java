...
/**
 * Martingale strategy:
 * - Opens at bar 0, doubles amount each time price drops a threshold from the last entry.
 * - BTC: fixed 2% drop threshold.
 * - Others: threshold widens with each additional position (2% * position count).
 * - Take profit when PnL >= 3.
 * - Stop loss when PnL < -balance (full loss).
 */
public class MartingaleStrategy extends AbstractBacktest {

    private final String symbol;

    public MartingaleStrategy(String symbol) {
        this.symbol = symbol;
    }
    ...
    public static void main(String[] args) {
        ...
        new MartingaleStrategy(symbol).run(symbol, interval, limit);
    }
}
